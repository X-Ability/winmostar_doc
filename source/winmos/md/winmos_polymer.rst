...
-------------------
   ...
   1) メインウィンドウにおいて登録したいモノマー（繰り返し単位）の構造を作成します。
   ...
   [Name]
      登録するモノマーの名前を入力します。
   [OK]
      モノマーを登録しウィンドウを閉じます。モノマーファイルが作成されます。
   [Cancel]
...
------------------------
   ...
   [non-bonded inter-atomic distance tolerance]
      非結合原子間距離の最小値の許容値を指定します。
      指定した許容値で作成できなかった場合は、徐々に許容値を小さくして作成するので、必ず許容値に収まっているとは限りません。
      許容値の変化はログに出力されます。
   [Build]
      シミュレーションセルを作成します。
   [Close]
