...
------------------------
   ...
   本機能は主に以下の2つの目的で使用されます。
      ...
      1. メインウインドウに表示されている分子の周りに溶媒分子を並べる
      2. 低分子を並べて液相を作成する
   ...
   作業用フォルダ以下の :file:`output.gro` が最終的に生成された分子構造を含むファイルになります。
      ...
      Add Displayed Molecule
         メインウインドウに表示されている分子を追加します。ボタンを押した後、追加する分子数を入力します。
         1個しか配置しない場合は、メインウインドウに表示されている分子については座標が固定された状態で他の分子が並べられます。
      Add Water
...
---------------------
   ...
   mol2形式で保存された分子を複数個、メインウインドウに表示されている構造に追加することができます。
   シミュレーションセルが作成されていない場合は、事前に :ref:`edit_createcell` または :ref:`md_solvate_buildcell` を使用して作成してください。
