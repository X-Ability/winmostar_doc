...
編集操作向けの元素を選択
-------------------------------------
   :ref:`edit_atom` や :ref:`edit_attribute_element` で適用される元素を選択します。
...
選択原子に付加（自動）
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   マーカーが付いた原子に水素原子を付加します。
   グループ選択されている場合はその原子に水素を付加します。
...
-------------------------------------
   ...
   .. hint::
       :kbd:`F6` 、 :guilabel:`Replace` ボタン、または原子を右クリックでも操作できます。
       
フラグメントを選択
-------------------------------------
   :ref:`edit_parts` で置換されるフラグメントを選択します。
...
.. _edit_group_rotate_slider:
   
グループを回転（数値を指定）
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   グループ選択された構造を、 :ref:`マーカー（赤太丸） <edit_top>` が付いた原子または幾何中心を中心に、スライダー操作まはた数値入力により回転させます。指定するのはオイラー角です。
   
   .. hint::
       :kbd:`Ctrl+F` でも操作できます。
   
.. _edit_group_rotate_orien:
...
密度を変更
-------------------------------------
   密度を指定して、シミュレーションセルと原子座標を相似的に拡大または縮小します。
