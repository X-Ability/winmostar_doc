...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   ...
   「分子構造的に等価な原子に同じ電荷を割り当てますか？」と聞かれ、 :guilabel:`はい` をクリックすると内部的にはacpypeを、 :guilabel:`いいえ` をクリックすると内部的にはAmberToolsを直接使ってRESP電荷を算出しています。基本的には :guilabel:`はい` をクリックする方を推奨しますが、大きい分子の場合などに処理が正常終了しない場合があり、そのような場合は :guilabel:`いいえ` をクリックしてください。両者の計算結果は、大きくは変化しないことが期待されます。
